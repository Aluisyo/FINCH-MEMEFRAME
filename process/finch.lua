...
> .load process/finch-token
> .load process/finch-staking
> .load process/finch.lua
...
function Man (name) 
  ...
  Join the FINCH community. Mint $FINCH Tokens using $CRED, then Stake them for voting on the Webpage to show
    on the FINCH page.
  ...
  ```
  Send({Target = FINCH, Action = "Vote", Side = "yay", TXID = "{TXID}" })
  ```

  ## Get-Votes

  ```
  Send({Target = FINCH, Action = "Get-Votes"})
  ```
